...
export default function DashboardAdmin() {
  ...
  const loadUsers = useCallback(async () => {
    try {
      ...
      const normalized = data.map(u => ({
        ...u,
        role: u.role === 'admin' ? 'Admin' : u.role === 'coach' ? 'Coach' : u.role === 'user' ? 'User' : u.role
      }))
      setUsers(normalized)
  ...
  const handleSaveUser = async (formData) => {
    try {
      if (selectedUser) {
        const payload = { name: formData.name, email: formData.email, role: formData.role.toLowerCase() }
        if (formData.password) payload.password = formData.password
        await updateUser(selectedUser.id, payload)
        Swal.fire('Actualizado', 'Usuario actualizado correctamente', 'success')
      } else {
        await createUser({ ...formData, role: formData.role.toLowerCase(), password: formData.password || '12345678' })
  ...
  const stats = [
    { icon: <UsersIcon size={22} />, label: 'Total usuarios', value: users.length, color: accent },
     { icon: <UsersIcon size={22} />, label: 'Coaches', value: users.filter(u => u.role.toLowerCase() === 'coach').length, color: 'var(--coach-color)' },
     { icon: <DumbbellIcon size={22} />, label: 'Deportes', value: sports.length, color: 'var(--gold)' },
     { icon: <ShieldIcon size={22} />, label: 'Administradores', value: users.filter(u => u.role.toLowerCase() === 'admin').length, color: '#8a5ab8' },
  ]
